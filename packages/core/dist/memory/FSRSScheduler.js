...
export class FSRSScheduler {
    params;
    clock;
    constructor(params = {}, clock = () => Date.now()) {
     ...
     */
    scheduleReview(state, recalled, rating) {
        const now = this.clock();
     ...
     * R(t) = (1 + t/(9*S))^(-1)
     */
    calculateRetention(stability, elapsedDays) {
        if (elapsedDays <= 0)
            return 1.0;
        if (stability <= 0)
            return 0.0;
     ...
     * interval = S * 9 * (1/R - 1)
     */
    calculateInterval(stability, requestedRetention) {
     ...
     */
    updateStability(stability, difficulty, elapsedDays, rating) {
 ...
 */
export function createFSRSScheduler(params = {}, clock = () => Date.now()) {
    return new FSRSScheduler(params, clock);
}
//# sourceMappingURL=FSRSScheduler.js.map