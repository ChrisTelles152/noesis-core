...
export class BKTEngine {
     ...
     */
    createModel(learnerId, skillGraph) {
     ...
     */
    createSkillProbability(skillId, timestamp) {
        return {
            skillId,
     ...
     */
    getUnmasteredSkills(model, threshold) {
        const unmastered = [];
     ...
     */
    initializeFromDiagnostic(model, diagnosticResults, timestamp) {
        const newSkillProbabilities = new Map(model.skillProbabilities);
 ...
 */
export function createBKTEngine(params = {}, clock = () => Date.now()) {
    return new BKTEngine(params, clock);
}
//# sourceMappingURL=BKTEngine.js.map