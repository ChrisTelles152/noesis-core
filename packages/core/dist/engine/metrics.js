 ...
 */
export function getLearnerMetrics(engine, learnerId, atTime, options) {
    ...
    const totalPracticeEvents = eventLog.filter((e) => e.type === 'practice' && e.learnerId === learnerId).length;
    ...
    const eventsPerSkill = Math.ceil(Math.log(1 - DEFAULT_MASTERY_THRESHOLD) / Math.log(1 - DEFAULT_P_LEARN));
