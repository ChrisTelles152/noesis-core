 ...
 */
export const DEFAULT_LAYERED_MASTERY_CONFIG = {
    learned: {
 ...
 */
export const NO_CHANNEL_MAPPING = {
...
export class LayeredMasteryModel {
    config;
    channelMapping;
    constructor(config = {}, channelMapping = NO_CHANNEL_MAPPING) {
     ...
     */
    classifySkill(skillId, channelStates, now) {
        ...
        const avgPMastery = channelsWithData.length > 0
        ...
        const secondaryStatus = secondary !== null ? channels.find((c) => c.channel === secondary) : undefined;
        ...
        const isMastered = masteredChannels.length >= 2 ||
    ...
    // =============================================================================
    checkLearned(state) {
        return (state.pMastery >= this.config.learned.pMasteryThreshold &&
            state.attempts >= this.config.learned.minAttempts);
    }
    checkMastered(state, now) {
        const blockers = [];
        const m = this.config.mastered;
        if (state.pMastery < m.pMasteryThreshold) {
            blockers.push(`pMastery ${(state.pMastery * 100).toFixed(1)}% < ${m.pMasteryThreshold * 100}%`);
        }
        ...
        if (state.correctDays.length < m.minCalendarDays) {
            blockers.push(`${state.correctDays.length} days with correct < ${m.minCalendarDays} required`);
        }
        const hoursSinceFirstSeen = (now - state.firstSeenAt) / MS_PER_HOUR;
        if (hoursSinceFirstSeen < m.coolingOffHours) {
            blockers.push(`Only ${hoursSinceFirstSeen.toFixed(1)}h since first seen, need ${m.coolingOffHours}h`);
        }
 ...
 * Factory.
 */
export function createLayeredMasteryModel(config = {}, channelMapping = NO_CHANNEL_MAPPING) {
    return new LayeredMasteryModel(config, channelMapping);
}
/**
 * Helper: build a SkillChannelMapping from a function.
 */
export function makeChannelMapping(fn) {
    return { forSkill: fn };
}
//# sourceMappingURL=LayeredMasteryModel.js.map