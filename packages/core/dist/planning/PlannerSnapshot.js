 ...
 */
import { BudgetedSessionPlanner, DEFAULT_SESSION_BUDGET_CONFIG, } from './BudgetedSessionPlanner.js';
/** Schema version of the snapshot shape. Bump on any breaking change. */
export const PLANNER_SNAPSHOT_VERSION = '1.0.0';
/**
 ...
 *                  config variants).
 */
export function planFromSnapshot(snapshot, planner) {
 ...
 */
export function serializePlannerSnapshot(snapshot) {
 ...
 */
export function deserializePlannerSnapshot(data) {
    const parsed = JSON.parse(data);
    if (parsed.version !== PLANNER_SNAPSHOT_VERSION) {
        throw new Error(`PlannerSnapshot version mismatch: expected ${PLANNER_SNAPSHOT_VERSION}, got ${parsed.version}`);
    }
...
//# sourceMappingURL=PlannerSnapshot.js.map