...
export class SessionPlannerImpl {
     ...
     */
    getNextAction(learnerModel, skillGraph, memoryStates, config) {
     ...
     */
    planSession(learnerModel, skillGraph, memoryStates, config) {
        const mergedConfig = { ...this.config, ...config };
        const actions = [];
        const now = learnerModel.lastUpdated;
        ...
        while (itemCount < mergedConfig.targetItems) {
            ...
            const nextAction = this.getNextActionExcluding(learnerModel, skillGraph, memoryStates, config, plannedSkills);
     ...
     */
    getTransferTestAction(learnerModel, skillGraph) {
        ...
        for (const skillId of skillOrder) {
            ...
            if (pMastery >= this.config.transferTestThreshold) {
                ...
                const passedTests = new Set(this.transferResults
                    .filter(r => r.passed)
                    .map(r => r.testId));
     ...
     */
    getNewSkillAction(learnerModel, skillGraph) {
        const skillOrder = skillGraph.getTopologicalOrder();
        const candidates = [];
        for (const skillId of skillOrder) {
            const pMastery = learnerModel.skillProbabilities.get(skillId)?.pMastery || 0;
     ...
     */
    getNextActionExcluding(learnerModel, skillGraph, memoryStates, config, excludeSkills) {
        ...
        const filteredModel = {
            ...learnerModel,
            skillProbabilities: new Map(Array.from(learnerModel.skillProbabilities.entries())
